...
from neo_api_client.core.settings import PROD_URL
...
class TotpAPI(object):
    ...
    def totp_login(self, mobile_number=None, ucc=None, totp=None):
        ...
        if 200 <= totp_login.status_code <= 299:
            self.api_client.configuration.view_token = totp_login_data.get("data").get(
                "token"
            )
    ...
    def totp_validate(self, mpin=None):
        ...
        if 200 <= totp_validate.status_code <= 299:
            self.api_client.configuration.edit_token = totp_validate_data.get(
                "data"
            ).get("token")
            self.api_client.configuration.edit_sid = totp_validate_data.get("data").get(
                "sid"
            )
            self.api_client.configuration.edit_rid = totp_validate_data.get("data").get(
                "rid"
            )
            self.api_client.configuration.serverId = totp_validate_data.get("data").get(
                "hsServerId"
            )
            self.api_client.configuration.data_center = totp_validate_data.get(
                "data"
            ).get("dataCenter")
            self.api_client.configuration.base_url = totp_validate_data.get("data").get(
                "baseUrl"
            )
