            ...
            if exchange_segment is not None:
                ...
                if exchange_segment.endswith("fo"):
                    if not (
                        ...
                        df["pExpiryDate"] = df["pExpiryDate"] + pd.to_timedelta(
                            315511200, unit="s"
                        )
                ...
                if symbol != "":
                    mask = (
                        df["pSymbolName"].str.lower().str.strip().str.contains(symbol)
                    )
                    df = df[mask]
                if option_type:
                ...
                if expiry:
                    ...
                    if len(list_expiry) > 2:
                        ...
                        df["pExpiryDate"] = pd.to_datetime(
                            df["pExpiryDate"], format="%d%b%Y"
                        )
                        df = df[
