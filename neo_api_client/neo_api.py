...
from neo_api_client.core import req_data_validation
from neo_api_client.core.api_client import ApiClient


class NeoAPI:
    """
    NeoAPI client for the Kotak Neo platform.

    This class wraps the lower-level neo_api_client APIs and provides a higher-level
    interface for trading operations and market data. It initializes with a
    NeoUtility-based configuration and an ApiClient and supports optional
    websocket callbacks and orderfeed subscriptions.

    Constructor:
        __init__(environment='uat', neo_fin_key=None, consumer_key=None)
        - environment (str): Environment name to connect to (e.g. 'uat' or 'prod').
        - neo_fin_key (str|None): Optional tracking Finkey.
        - consumer_key (str|None): Optional consumer key for API authentication.

    Attributes:
        configuration: neo_api_client.NeoUtility configuration instance.
        api_client: ApiClient instance created from the configuration.
        NeoWebSocket: Websocket helper instance (set after subscribe) or None.
        on_message, on_error, on_close, on_open: optional callback callables for
            websocket events.

    Important notes:
        - Many methods (order placement, portfolio, positions, websocket subscribe,
          orderfeed subscription, etc.) require the 2FA/TOTP login flow to be
          completed. Internally these methods check configuration.edit_token and
          configuration.edit_sid before proceeding and will return an error message
          if the login flow is not complete.
        - Methods generally return the underlying API response or a dict like
          {'Error': <exception>} on failure.

    Main supported operations include:
        place_order, cancel_order, cancel_cover_order, cancel_bracket_order,
        order_report, order_history, trade_report, modify_order, positions,
        holdings, margin_required, scrip_master, limits, search_scrip,
        subscribe, un_subscribe, totp_login, totp_validate, quotes,
        subscribe_to_orderfeed, unsubscribe_orderfeed, logout, help
    """

    def __init__(
        self, environment="uat", neo_fin_key=None, consumer_key=None
    ):
        """
    ...
    def place_order(
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                ...
                exchange_segment = neo_api_client.settings.exchange_segment[
                    exchange_segment
                ]
            ...
            return {
                "Error Message": "Complete the TOTP login process before accessing this application"
            }
    ...
    def cancel_order(self, order_id, amo="NO", isVerify=False):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                neo_api_client.core.req_data_validation.cancel_order_validation(
                    order_id
                )
                cancel_order = neo_api_client.CancelOrder(
                    self.api_client
                ).order_cancelling(order_id=order_id, isVerify=isVerify, amo=amo)
                return cancel_order
            except Exception as e:
                return {"Error": e}
        else:
    ...
    def cancel_cover_order(self, order_id, amo="NO", isVerify=False):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                ...
                cancel_order = neo_api_client.CancelOrder(
                    self.api_client
                ).cover_order_cancelling(order_id=order_id, isVerify=isVerify, amo=amo)
                return cancel_order
            except Exception as e:
                return {"Error": e}
        else:
    ...
    def order_report(self):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                order_list = neo_api_client.OrderReportAPI(
                    self.api_client
                ).ordered_books()
                return order_list
            except Exception as e:
    ...
    def order_history(self, order_id):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                neo_api_client.core.req_data_validation.order_history_validation(
                    order_id
                )
                history_list = neo_api_client.OrderHistoryAPI(
                    self.api_client
                ).ordered_history(order_id=order_id)
                return history_list
            except Exception as e:
                return {"Error": e}
        else:
    ...
    def trade_report(self, order_id=None):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                filtered_trades = neo_api_client.TradeReportAPI(
                    self.api_client
                ).trading_report(order_id=order_id)
    ...
    def modify_order(
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            if (
                ...
                order_type = neo_api_client.settings.order_type[order_type]
                try:
                    quick_modify = neo_api_client.ModifyOrder(
                        self.api_client
                    ).quick_modification(
                        order_id=order_id,
                        price=price,
                    ...
                    return {
                        "Error": "Exception has been occurred while connecting to API"
                    }
            elif (
                order_id
                and not instrument_token
                and not exchange_segment
                and not trading_symbol
            ):
                try:
                    modify_order = neo_api_client.ModifyOrder(
                        self.api_client
                    ).modification_with_orderid(
                        order_id=order_id,
                        price=price,
                        order_type=order_type,
                        quantity=quantity,
    ...
    def positions(self):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                position_list = neo_api_client.PositionsAPI(
                    self.api_client
                ).position_init()
    ...
    def holdings(self):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                portfolio_list = neo_api_client.PortfolioAPI(
                    self.api_client
                ).portfolio_holdings()
    ...
    def margin_required(
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                neo_api_client.core.req_data_validation.margin_validation(
                    exchange_segment,
                    price,
                    order_type,
                    product,
                ...
                product = neo_api_client.settings.product[product]
                order_type = neo_api_client.settings.order_type[order_type]
                margin_required = neo_api_client.MarginAPI(self.api_client).margin_init(
                    exchange_segment=exchange_segment,
                    price=price,
                    order_type=order_type,
                    product=product,
    ...
    def scrip_master(self, exchange_segment=None):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                scrip_list = neo_api_client.ScripMasterAPI(
                    self.api_client
                ).scrip_master_init(exchange_segment=exchange_segment)
                return scrip_list
            except Exception as e:
    ...
    def limits(self, segment="ALL", exchange="ALL", product="ALL"):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:
                neo_api_client.core.req_data_validation.limits_validation(
                    segment, exchange, product
                )

                limits_list = neo_api_client.LimitsAPI(self.api_client).limit_init(
    ...
    def search_scrip(
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            ...
            try:
                ...
                scrip_list = neo_api_client.ScripSearch(self.api_client).scrip_search(
                    exchange_segment=exchange_segment,
                    symbol=symbol,
                    expiry=expiry,
    ...
    def check_callbacks(self):
        show_warning = (
            not self.on_close
            or not self.on_open
            or not self.on_message
            or not self.on_error
        )
        if show_warning:
            warnings = "Warning!\n"
    ...
    def subscribe(self, instrument_tokens, isIndex=False, isDepth=False):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            if not self.NeoWebSocket:
                ...
                self.NeoWebSocket = neo_api_client.NeoWebSocket(
    ...
    def un_subscribe(self, instrument_tokens, isIndex=False, isDepth=False):
        ...
        if self.configuration.edit_token and self.configuration.edit_sid:
            ...
            raise ValueError(
                "Please complete the Login Flow to Un_Subscribe the Scrips"
            )
    ...
    def unsubscribe_orderfeed(self):
        """Safely unsubscribe from the order feed websocket."""
        if self.configuration.edit_token and self.configuration.edit_sid:
            try:

                if self.NeoWebSocket and getattr(
                    self.NeoWebSocket, "hsiWebsocket", None
                ):
                    # self.NeoWebSocket.unsubscribe_order_feed()
                    try:
                        self.NeoWebSocket.hsiWebsocket.close()
                    except Exception:
                        pass
                    try:
                        self.NeoWebSocket.hsiWebsocket = None
                        self.NeoWebSocket.is_hsi_open = 0
                    except Exception:
                        pass
                    return {"State": "OK", "message": "Unsubscribed from order feed"}
                return {
                    "State": "NOT_OK",
                    "message": "No active orderfeed subscription",
                }
            except Exception as e:
                return {"Error": e}
        else:
    ...
    def totp_login(self, mobile_number=None, ucc=None, totp=None):
        ...
        if not mobile_number or not ucc or not totp:
            ...
            return error
        totp_login = neo_api_client.TotpAPI(self.api_client).totp_login(
    ...
    def quotes(self, instrument_tokens=None, quote_type=None):
        ...
        if not instrument_tokens:
            error = {
                "error": [
                    {"message": "Validation Errors! instrument_tokens are missing"}
                ]
            }
            return error
        quotes_response = neo_api_client.QuotesAPI(self.api_client).get_quotes(
