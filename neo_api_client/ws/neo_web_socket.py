...
from neo_api_client.core.settings import (
    stock_key_mapping,
    QuotesChannel,
    ReqTypeValues,
    index_key_mapping,
...
class NeoWebSocket:
    ...
    def start_hsi_ping_thread(self, interval: float = 30.0):
        def _ping_loop():
            while True:
                # wait for an open websocket
                if not self.is_hsi_open or self.hsiWebsocket is None:
                    time.sleep(1)
                    continue

                payload = {"type": "HB"}
                try:
                    ws = self.hsiWebsocket
                    # optional library-specific connected check (websocket-client exposes ws.sock.connected)
                    if hasattr(ws, "sock") and not getattr(ws.sock, "connected", True):
                        time.sleep(1)
                        continue

                    ws.send(json.dumps(payload))
                except Exception:
                    # swallow/send to on_error to avoid killing the thread
                    try:
                        if self.on_error:
                            self.on_error(Exception("HSI ping failed"))
                    except Exception:
                        pass
                time.sleep(interval)

        t = threading.Thread(target=_ping_loop, name="hsi_ping_thread", daemon=True)
        t.start()
        self.hsi_thread = t
        return t
            ...
            if type(message) == str:
                ...
                if req_type == "unsub":
                    if len(self.un_sub_channel_token) > 0 and self.un_sub_channel:
                        ...
                        self.remove_items(
                            self.un_sub_channel_token[self.un_sub_channel]
                        )
                ...
                if (
                    request_type
                    and request_type == "SNAP"
                    and (len(self.quotes_arr) >= 1)
                ):
                    ...
                    if len(out_list) > 0:
                        ...
                        quote_message = self.response_format(
                            out_list, quote_type=quote_type
                        )
                        if self.on_message:
                            self.on_message({"type": "quotes", "data": quote_message})
                        self.quotes_arr = []
                if len(self.sub_list) >= 1 and self.is_message_for_subscription(
                    message
                ):
                    if self.on_message:
    ...
    def remove_items(self, un_sub_json):
        for unsubscribe_token in un_sub_json:
            token_value = unsubscribe_token[list(unsubscribe_token.keys())[0]][
                "instrument_token"
            ]
            segment_value = unsubscribe_token[list(unsubscribe_token.keys())[0]][
                "exchange_segment"
            ]
            ...
            self.sub_list = [
                token for token in self.sub_list if token != unsubscribe_token
            ]
    ...
    def get_formatted_data(self, instrument_tokens):
        ...
        for item in instrument_tokens:
            for k, v in item.items():
                if (
                    type(v) == dict
                    and "exchange_segment" in v.keys()
                    and "instrument_token" in v.keys()
                ):
                    if scrips != "":
                        scrips += "&"
    ...
    def format_tokens_live(self, instrument_tokens):
        scrips = ""
        if (
            type(instrument_tokens) == dict
            and "exchange_segment" in instrument_tokens.keys()
            and "instrument_token" in instrument_tokens.keys()
        ):
            if scrips != "":
                scrips += "&"
            scrips += (
    ...
    def format_un_sub_list(self, instrument_tokens):
        ...
        for instrument_token in instrument_tokens:
            if (
                type(instrument_token) == dict
                and "exchange_segment" in instrument_token.keys()
                and "instrument_token" in instrument_token.keys()
            ):
                if scrips != "":
                    scrips += "&"
                scrips += (
    ...
    def call_quotes(self):
        ...
        req_params = json.dumps(
            {"type": scrip_type, "scrips": scrips, "channelnum": QuotesChannel}
        )
    ...
    def quote_type_validation(self, quote_type):
        Q_type = True
        if quote_type:
            if str(quote_type).strip().lower() not in [
                "market_depth",
                "ohlc",
                "ltp",
                "52w",
    ...
    def prepare_un_sub(self):
        ...
        for key, value in self.channel_tokens.items():
            ...
            for item in value:
                ...
                new_value = [
                    {list(item.values())[0]["instrument_token"]: list(item.values())[0]}
                ]
            ...
            for item in response_data:
                if type(item) == dict:
                    ...
                    if quote_type:
                        if quote_type.strip().lower() != "market_depth":
                            out_resp.append(
                                self.quote_type_filter(new_dict, quote_type)
                            )
                    else:
        ...
        if self.quotes_index:
            if len(response_data) >= 1:
                for item in response_data:
                    if type(item) == dict:
                        new_dict = {
                            index_key_mapping.get(k, k): v for k, v in item.items()
                        }
                        for key in list(new_dict.keys()):
    ...
    def unsubscribe_order_feed(self):
        if self.hsiWebsocket and self.is_hsi_open == 1:
            try:
